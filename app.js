// Данные справочника кладовщика
const warehouseData = {
    chapters: [
        {
            id: 'chapter1',
            title: 'Процесс приемки',
            icon: '🚛',
            description: 'Полный процесс приемки товарно-материальных ценностей',
            subchapters: [
                { id: '1.1', title: 'Заезд ТС на площадку' },
                { id: '1.2', title: 'Правила предоставления информации в Отдел Учета' },
                { id: '1.3', title: 'Проверка Документов перед выгрузкой' },
                { id: '1.4', title: 'Начало выгрузки' },
                { id: '1.5', title: 'Работа с Несоответствиями при Приемке ТМЦ' },
                { id: '1.6', title: 'Составление Акта М7' },
                { id: '1.7', title: 'Выгрузка ТС' },
                { id: '1.8', title: 'Составление Акта о Повреждении ТМЦ' },
                { id: '1.9', title: 'Предоставление информации о выгруженном ТС в Отдел Учета' }
            ]
        },
        {
            id: 'chapter2',
            title: 'Хранение материала',
            icon: '📦',
            description: 'Правила хранения и перемещения ТМЦ',
            subchapters: [
                { id: '2.1', title: 'Общие положения по хранению ТМЦ' },
                { id: '2.2', title: 'Коды Хранения ТМЦ' },
                { id: '2.3', title: 'Перемещение ТМЦ между складами' },
                { id: '2.4', title: 'Составление Акта о Повреждении ТМЦ' },
                { id: '2.5', title: 'Перенаправление ТМЦ со склада' }
            ]
        },
        {
            id: 'chapter3',
            title: 'Выдача материала в монтаж',
            icon: '🔧',
            description: 'Процедуры выдачи материалов для монтажных работ',
            subchapters: [
                { id: '3.1', title: 'Выдача по Заявке на Выдачу целыми ГМ' },
                { id: '3.2', title: 'Частичная Выдача' },
                { id: '3.3', title: 'Составление Акта Осмотра' },
                { id: '3.4', title: 'Составление Акта о Повреждении ТМЦ' }
            ]
        },
        {
            id: 'chapter4',
            title: 'Документация и правила ее оформления',
            icon: '📋',
            description: 'Правила оформления всех видов документов',
            subchapters: [
                { id: '4.1', title: 'Правила оформления ТН' },
                { id: '4.2', title: 'Правила Оформления CMR' },
                { id: '4.3', title: 'Правила заполнения Ведомости на перемещение' },
                { id: '4.4', title: 'Содержание Упаковочного Листа (УЛ)' },
                { id: '4.5', title: 'Содержание Ведомости Грузового места' },
                { id: '4.6', title: 'Содержание Shipping Mark' },
                { id: '4.7', title: 'Содержание Заявки на Выдачу' },
                { id: '4.8', title: 'Содержание и Правила заполнения Пропуска на Вывоз' }
            ]
        },
        {
            id: 'chapter5',
            title: 'Информационные группы',
            icon: '💬',
            description: 'Функционал информационных групп и примеры',
            subchapters: [
                { id: '5.1', title: 'Оперуполномоченные' },
                { id: '5.2', title: 'БЕЗУМИЕ. Площадка' },
                { id: '5.3', title: 'Информация. Читай меня!' },
                { id: '5.4', title: 'Документы на проверку' },
                { id: '5.5', title: 'Акты «М-7»' },
                { id: '5.6', title: 'Для ТН для учета' },
                { id: '5.7', title: 'Проверка заявок перед выдачей' },
                { id: '5.8', title: 'Выдача ТМЦ по заявкам' },
                { id: '5.9', title: 'Потеряшки' }
            ]
        },
        {
            id: 'chapter6',
            title: 'Общие Хозяйственные Вопросы',
            icon: '🏢',
            description: 'Хозяйственные вопросы и процедуры',
            subchapters: [
                { id: '6.1', title: 'Заявки на канцелярию и расходные материалы' },
                { id: '6.2', title: 'Время труда и отдыха' },
                { id: '6.3', title: 'Оформление отпуска' },
                { id: '6.4', title: 'Передача документов в учет' }
            ]
        }
    ],
    content: {
        '1.1': {
            title: 'Заезд ТС на площадку',
            content: `
                <h3>Направление ТС</h3>
                <p>ТС (Транспортные Средства) направляются на соответствующие локации диспетчерами.</p>
                
                <h3>Файл по поступающим ТС</h3>
                <p>Файл находится в общем доступе по ссылке:</p>
                <p><a href="https://docs.google.com/spreadsheets/d/1_3yw0wras6aP1Mfi_0k157Ja9whFUSgZk6vMLXsOB8A/edit?usp=sharing" target="_blank">Открыть файл ТС</a></p>
                <p>Также доступно по QR коду.</p>
                
                <h3>Контакты диспетчеров:</h3>
                <ul>
                    <li>+7 (981) 234-85-59 Полозова Анастасия</li>
                    <li>+7 (981) 149-36-86 Стрюкова Ольга</li>
                    <li>+7 (981) 723-28–91 Амелина Ксения</li>
                </ul>
                
                <h3>Запрос дополнительной техники</h3>
                <p>Запрос дополнительной техники для выгрузки/погрузки/работы на складской площадке также осуществляется через диспетчеров.</p>
            `
        },
        '1.2': {
            title: 'Правила предоставления информации в Отдел Учета',
            content: `
                <h3>Требования к информации</h3>
                <p>Информация, предоставляемая в Отдел Учета должна быть:</p>
                <ul>
                    <li>Понятна</li>
                    <li>Хорошо читаема</li>
                    <li>Конкретна</li>
                    <li>Однозначна</li>
                </ul>
                
                <h3>Требования к фотографиям</h3>
                <p>Фотографии необходимо предоставлять в HD качестве, без необходимости переворачивать фото для прочтения информации.</p>
                
                <h3>Важно!</h3>
                <p>Если Вы видите, что Китайские Коллеги нарушают эти требования – необходимо сделать им замечание с просьбой предоставлять информацию в соответствии с нашими требованиями.</p>
                
                <p>Использование нестандартных символов, замена букв на цифры и подобные манипуляции ведут к ошибкам в работе с 1С, искажению информации и увеличению трудозатрат ваших Коллег.</p>
                
                <div class="image-container">
                    <img src="images/image_4_2.png" alt="Пример неверного оформления" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример неверного оформления</em></p>
                </div>
            `
        },
        '1.3': {
            title: 'Проверка Документов перед выгрузкой',
            content: `
                <h3>Проверка документов</h3>
                <p>Если поставщик материала присутствует в списке на проверку документов, то перед выгрузкой ТС необходимо предоставить фото лицевой страницы ТН в информационную группу в мессенджере MAX «Документы на проверку».</p>
                
                <h3>Список поставщиков</h3>
                <p>Список публикуется в группе «Информация. Читай меня». Актуальную версию списка искать в группе.</p>
                
                <h3>Комплект документов</h3>
                <p>Комплект документов должен состоять из:</p>
                <ul>
                    <li>ТН/CMR</li>
                    <li>УЛ (Упаковочный Лист)</li>
                </ul>
                
                <p>Допускается приемка ТС по одной ТН без УЛ, если Уникальные идентификаторы грузового места прописаны в ТН.</p>
                
                <h3>Важно!</h3>
                <p>Кладовщик НЕ подписывает и НЕ ставит печати на листы простоя, путевые листы, акты или иные документы по запросу водителей при приемке материалов, если иные распоряжения не поступали от руководства.</p>
                
                <div class="image-container">
                    <img src="images/image_4_3.png" alt="Пример проверки документов" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример проверки документов</em></p>
                </div>
            `
        },
        '1.4': {
            title: 'Начало выгрузки',
            content: `
                <h3>Порядок выгрузки</h3>
                <p>Если ТН скидываются в группу «Документы на проверку», то необходимо дождаться подтверждения от Отдела Учета о возможности выгрузить данное ТС.</p>
                
                <p>Если информация о поставке не занесена в 1С, либо есть другие препятствия к выгрузке, то выгрузка не производится до уведомления Отдела Учета о том, что можно выгружать ТС.</p>
                
                <h3>Беспрепятственная выгрузка</h3>
                <p>Если ТН НЕ НУЖНО скидывать в группу «Документы на проверку», то есть поставщик не присутствует в списке на проверку документов, то ТС выгружается беспрепятственно в порядке очереди.</p>
                
                <p>Очередность выгрузки можно посмотреть в файле по поступающим ТС от диспетчеров.</p>
                
                <h3>Осмотр ТМЦ</h3>
                <p>После заезда ТС на площадку необходимо осмотреть ТМЦ на предмет повреждений и соответствия сопроводительной документации.</p>
            `
        },
        '1.5': {
            title: 'Работа с Несоответствиями при Приемке ТМЦ',
            content: `
                <h3>Виды несоответствий</h3>
                <ol>
                    <li>Повреждения упаковки и самих ТМЦ</li>
                    <li>Недостачи</li>
                    <li>Излишки</li>
                    <li>Пересорт</li>
                    <li>Отсутствие маркировки (УЛ, заводская маркировка и т.д.)</li>
                </ol>
                
                <h3>Действия при обнаружении несоответствий</h3>
                <p>При обнаружении несоответствий после заезда ТС на площадку, необходимо:</p>
                <ol>
                    <li>Зафиксировать эти несоответствия на фото</li>
                    <li>С подробным описанием скинуть данные в группу «М7»</li>
                </ol>
                
                <p>Отдел Учета подготовит Акт М7 и направит его в адрес поставщика.</p>
                
                <h3>Важно!</h3>
                <p>Повреждения ТМЦ необходимо зафиксировать на фото НА ТС ДО ВЫГРУЗКИ (если заметить повреждения на этом этапе представляется возможным).</p>
            `
        },
        '1.6': {
            title: 'Составление Акта М7',
            content: `
                <h3>Когда составляется Акт М-7</h3>
                <p>Акт М-7 составляется при наличии несоответствий:</p>
                <ol>
                    <li>Повреждения упаковки и самих ТМЦ</li>
                    <li>Недостачи</li>
                    <li>Излишки</li>
                    <li>Пересорт</li>
                    <li>Отсутствие маркировки (УЛ, заводская маркировка и т.д.)</li>
                </ol>
                
                <h3>Требования к информации</h3>
                <p>При подаче информации в Отдел Учета кладовщик должен убедиться в полноте, понятности, читаемости, конкретности и однозначности предоставляемой информации.</p>
                
                <h3>Формат предоставления информации</h3>
                <p>Для составления Акта М-7 кладовщик обязан предоставить фото и информацию в группу «М-7» в формате:</p>
                <ol>
                    <li>Фото лицевой страницы ТН, где видны номер ТН, ФИО водителя</li>
                    <li>Фото тягача ТС с читаемым гос номером</li>
                    <li>Фото несоответствия</li>
                    <li>Текстом прописан номер ТН, дата и время начала и окончания выгрузки</li>
                    <li>Текстом подробно описано несоответствие</li>
                </ol>
                
                <h3>Описание несоответствий</h3>
                <ul>
                    <li>Если недостача – в каком количестве и какие номера ГМ недостает</li>
                    <li>Если излишек – какое конкретное ГМ лишнее</li>
                    <li>Если повреждение – какое повреждение какого ГМ и т.д.</li>
                </ul>
                
                <div class="image-container">
                    <img src="images/image_4_4.png" alt="Пример составления Акта М7" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример составления Акта М7</em></p>
                </div>
            `
        },
        '1.7': {
            title: 'Выгрузка ТС',
            content: `
                <h3>Заказ техники</h3>
                <p>Техника для выгрузки ТС на складской площадке заказывается диспетчерами вечером предыдущего дня.</p>
                <p>Если необходимо увеличить количество техники - обращаться к диспетчерам.</p>
                
                <h3>Место выгрузки</h3>
                <p>Место для выгрузки определяется кладовщиком самостоятельно, если другое не определено руководством.</p>
                <p>Место выгрузки также определяется кодом хранения материала.</p>
                
                <h3>Коды хранения</h3>
                <p>Информация по кодам хранения присутствует в Упаковочном Листе и в Ведомости Грузового Места:</p>
                <ul>
                    <li><strong>Х</strong> - для хранения на открытой площадке, не под навесом</li>
                    <li><strong>ХХ</strong> - для хранения на открытой мощеной площадке, под навесом</li>
                    <li><strong>ХХХ</strong> - для хранения в закрытом, сухом, неотапливаемом помещении</li>
                    <li><strong>ХХХХ</strong> - для хранения в закрытом, сухом, отапливаемом / охлаждаемом помещении</li>
                    <li><strong>ХХХХХ</strong> - для перевозки на отапливаемых транспортных средствах</li>
                </ul>
                
                <h3>Правила выгрузки</h3>
                <ul>
                    <li>Выгрузка ХХ осуществляется с последующим укрытием тентами по умолчанию, если не поступало иных распоряжений</li>
                    <li>Выгрузка ХХХ при наличии места в крытых складах или контейнерах СТРОГО ЗАПРЕЩЕНА без согласования Руководителя склада либо его Заместителя</li>
                    <li>Выгрузка ХХХХ при наличии места в крытых отапливаемых складах СТРОГО ЗАПРЕЩЕНА без согласования Руководителя склада либо его Заместителя</li>
                </ul>
                
                <h3>Повреждения при ПРР</h3>
                <p>Если в ходе выгрузки груз был поврежден при ПРР, то в этом случае необходимо составить Акт о Повреждении ТМЦ.</p>
            `
        },
        '1.8': {
            title: 'Составление Акта о Повреждении ТМЦ',
            content: `
                <h3>Информация о повреждении</h3>
                <p>Информацию о Повреждении ТМЦ при ПРР необходимо направлять в группу Оперуполномоченные в формате:</p>
                
                <ol>
                    <li>Фото повреждения</li>
                    <li>Фото пропуска стропальщиков/оператора погрузчика, которые производил ПРР</li>
                    <li>Фото ТН, по которой пришел груз и фото ТС</li>
                    <li>Описание как именно произошло повреждение ТМЦ</li>
                    <li>Описание ТМЦ – номер ГМ, УЛ, количество поврежденного ТМЦ</li>
                </ol>
                
                <h3>Обработка акта</h3>
                <p>На основании вышеизложенных данных Отдел Учета подготавливает Акт о Повреждении ТМЦ и направляет полевому сотруднику на подпись.</p>
            `
        },
        '1.9': {
            title: 'Предоставление информации о выгруженном ТС в Отдел Учета',
            content: `
                <h3>Своевременность предоставления информации</h3>
                <p>По окончании выгрузки ТС, необходимо направить информацию в Отдел Учета.</p>
                
                <p>Отдел Учета работает в потоковом режиме, и делает проводки в момент поступления информации в группах.</p>
                
                <p>Все ТН должны быть оприходованы в 1С до конца рабочего дня. В связи с этим, информацию необходимо отправлять сразу по окончании выгрузки и не копить ТН, чтобы обеспечить непрерывный и равномерный процесс учета материалов в течение рабочего дня.</p>
                
                <h3>Требования к информации</h3>
                <p>При подаче информации в Отдел Учета кладовщик должен убедиться в полноте, понятности, читаемости, конкретности и однозначности предоставляемой информации.</p>
                
                <h3>Формат предоставления информации</h3>
                <p>Информация о выгруженном ТС должна предоставляться в формате:</p>
                <ol>
                    <li>Фото лицевой страницы ТН</li>
                    <li>Фото выгруженного груза на лоте хранения</li>
                    <li>Фото упаковочных листов, либо ведомостей грузовых мест, размещенных на ТМЦ. Если этого нет (в случае, например, с трубной продукцией) то предоставляется фото заводской маркировки</li>
                    <li>Текстом прописывается номер ТН, количество ГМ и номера этих ГМ, если на лицевой странице ТН и УЛ эта информация отсутствует</li>
                    <li>Текстом прописывается склад, на котором была произведена выгрузка и лот хранения</li>
                    <li>В случае если составлен М-7 дописать «Акт М-7»</li>
                </ol>
                
                <div class="image-container">
                    <img src="images/image_5_1.png" alt="Пример предоставления информации" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример предоставления информации</em></p>
                </div>
                
                <p><strong>На этом процесс приемки закончен.</strong></p>
            `
        },
        // Глава 2: Хранение материала
        '2.1': {
            title: 'Общие положения по хранению ТМЦ',
            content: `
                <h3>Основные принципы хранения</h3>
                <p>Товарно-материальные ценности должны храниться в соответствии с их характеристиками и требованиями производителя.</p>
                
                <h3>Классификация по условиям хранения</h3>
                <ul>
                    <li><strong>Х</strong> - открытая площадка, не под навесом</li>
                    <li><strong>ХХ</strong> - открытая мощенная площадка, под навесом</li>
                    <li><strong>ХХХ</strong> - закрытое, сухое, неотапливаемое помещение</li>
                    <li><strong>ХХХХ</strong> - закрытое, сухое, отапливаемое/охлаждаемое помещение</li>
                    <li><strong>ХХХХХ</strong> - перевозка на отапливаемых транспортных средствах</li>
                </ul>
                
                <h3>Контроль условий хранения</h3>
                <p>Необходимо регулярно контролировать условия хранения и состояние ТМЦ.</p>
            `
        },
        '2.2': {
            title: 'Коды Хранения ТМЦ',
            content: `
                <h3>Система кодирования</h3>
                <p>Коды хранения указываются в упаковочных листах и ведомостях грузовых мест.</p>
                
                <div class="image-container">
                    <img src="images/image_7_1.png" alt="Пример кодов хранения" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример кодов хранения</em></p>
                </div>
                
                <h3>Соблюдение кодов</h3>
                <p>Строго соблюдайте указанные коды хранения для обеспечения сохранности ТМЦ.</p>
            `
        },
        '2.3': {
            title: 'Перемещение ТМЦ между складами',
            content: `
                <h3>Правила перемещения</h3>
                <p>Перемещение ТМЦ между складами осуществляется по соответствующим документам.</p>
                
                <h3>Документооборот</h3>
                <p>Все перемещения должны быть задокументированы и отражены в учетной системе.</p>
            `
        },
        '2.4': {
            title: 'Составление Акта о Повреждении ТМЦ',
            content: `
                <h3>При обнаружении повреждений</h3>
                <p>При обнаружении повреждений ТМЦ при хранении необходимо составить соответствующий акт.</p>
                
                <h3>Процедура составления</h3>
                <p>Акт составляется аналогично процедуре при приемке ТМЦ.</p>
            `
        },
        '2.5': {
            title: 'Перенаправление ТМЦ со склада',
            content: `
                <h3>Перенаправление для выгрузки</h3>
                <p>ТМЦ могут быть перенаправлены со склада для выгрузки на территории завода.</p>
                
                <h3>Документооборот</h3>
                <p>Все перенаправления должны быть оформлены соответствующими документами.</p>
            `
        },
        // Глава 3: Выдача материала в монтаж
        '3.1': {
            title: 'Выдача по Заявке на Выдачу целыми ГМ',
            content: `
                <h3>Процедура выдачи</h3>
                <p>Выдача ТМЦ осуществляется по заявкам на выдачу целыми грузовыми местами.</p>
                
                <div class="image-container">
                    <img src="images/image_9_1.png" alt="Пример заявки на выдачу" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример заявки на выдачу</em></p>
                </div>
                
                <h3>Проверка документов</h3>
                <p>Перед выдачей необходимо проверить все документы и соответствие ТМЦ.</p>
            `
        },
        '3.2': {
            title: 'Частичная Выдача',
            content: `
                <h3>Условия частичной выдачи</h3>
                <p>Частичная выдача ТМЦ возможна при наличии соответствующих разрешений.</p>
                
                <h3>Документооборот</h3>
                <p>Все частичные выдачи должны быть оформлены соответствующими актами.</p>
            `
        },
        '3.3': {
            title: 'Составление Акта Осмотра',
            content: `
                <h3>Акт осмотра</h3>
                <p>При выдаче ТМЦ составляется акт осмотра для фиксации состояния материалов.</p>
                
                <div class="image-container">
                    <img src="images/image_12_1.png" alt="Пример акта осмотра" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример акта осмотра</em></p>
                </div>
            `
        },
        '3.4': {
            title: 'Составление Акта о Повреждении ТМЦ',
            content: `
                <h3>Повреждения при выдаче</h3>
                <p>При обнаружении повреждений ТМЦ при выдаче составляется соответствующий акт.</p>
                
                <h3>Процедура</h3>
                <p>Процедура аналогична составлению актов при приемке и хранении.</p>
            `
        },
        // Глава 4: Документация
        '4.1': {
            title: 'Правила оформления ТН',
            content: `
                <h3>Транспортная накладная</h3>
                <p>ТН должна содержать всю необходимую информацию о грузе и перевозке.</p>
                
                <div class="image-container">
                    <img src="images/image_13_1.png" alt="Пример ТН" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример оформления ТН</em></p>
                </div>
                
                <h3>Обязательные реквизиты</h3>
                <ul>
                    <li>Номер ТН</li>
                    <li>Дата составления</li>
                    <li>Данные отправителя и получателя</li>
                    <li>Описание груза</li>
                    <li>Количество и вес</li>
                </ul>
            `
        },
        '4.2': {
            title: 'Правила Оформления CMR',
            content: `
                <h3>Международная накладная CMR</h3>
                <p>CMR используется для международных перевозок грузов.</p>
                
                <div class="image-container">
                    <img src="images/image_13_2.png" alt="Пример CMR" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример оформления CMR</em></p>
                </div>
                
                <h3>Особенности заполнения</h3>
                <p>CMR заполняется на нескольких языках и содержит дополнительную информацию для международных перевозок.</p>
            `
        },
        '4.3': {
            title: 'Правила заполнения Ведомости на перемещение',
            content: `
                <h3>Ведомость на перемещение и приемку ТМЦ</h3>
                <p>Документ используется для учета перемещений ТМЦ между складами.</p>
                
                <div class="image-container">
                    <img src="images/image_14_1.png" alt="Пример ведомости" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример ведомости на перемещение</em></p>
                </div>
            `
        },
        '4.4': {
            title: 'Содержание Упаковочного Листа (УЛ)',
            content: `
                <h3>Упаковочный лист</h3>
                <p>УЛ содержит детальную информацию о содержимом каждого грузового места.</p>
                
                <div class="image-container">
                    <img src="images/image_17_1.png" alt="Пример УЛ" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример упаковочного листа</em></p>
                </div>
                
                <h3>Обязательная информация</h3>
                <ul>
                    <li>Номер грузового места</li>
                    <li>Описание ТМЦ</li>
                    <li>Количество</li>
                    <li>Код хранения</li>
                    <li>Вес и габариты</li>
                </ul>
            `
        },
        '4.5': {
            title: 'Содержание Ведомости Грузового места',
            content: `
                <h3>Ведомость грузового места</h3>
                <p>Документ содержит информацию о всех грузовых местах в поставке.</p>
                
                <div class="image-container">
                    <img src="images/image_18_1.png" alt="Пример ведомости ГМ" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример ведомости грузового места</em></p>
                </div>
            `
        },
        '4.6': {
            title: 'Содержание Shipping Mark',
            content: `
                <h3>Маркировка груза</h3>
                <p>Shipping Mark содержит информацию для идентификации и обработки груза.</p>
                
                <div class="image-container">
                    <img src="images/image_19_1.png" alt="Пример маркировки" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример маркировки груза</em></p>
                </div>
            `
        },
        '4.7': {
            title: 'Содержание Заявки на Выдачу',
            content: `
                <h3>Заявка на выдачу ТМЦ</h3>
                <p>Документ для оформления выдачи материалов со склада.</p>
                
                <div class="image-container">
                    <img src="images/image_20_1.png" alt="Пример заявки" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример заявки на выдачу</em></p>
                </div>
            `
        },
        '4.8': {
            title: 'Содержание и Правила заполнения Пропуска на Вывоз',
            content: `
                <h3>Пропуск на вывоз</h3>
                <p>Документ для оформления вывоза ТМЦ с территории склада.</p>
                
                <div class="image-container">
                    <img src="images/image_21_1.png" alt="Пример пропуска" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример пропуска на вывоз</em></p>
                </div>
            `
        },
        // Глава 5: Информационные группы
        '5.1': {
            title: 'Оперуполномоченные',
            content: `
                <h3>Группа Оперуполномоченные</h3>
                <p>Информационная группа для оперативного решения вопросов.</p>
                
                <div class="image-container">
                    <img src="images/image_22_1.png" alt="Пример группы" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример информационной группы</em></p>
                </div>
            `
        },
        '5.2': {
            title: 'БЕЗУМИЕ. Площадка',
            content: `
                <h3>Группа БЕЗУМИЕ. Площадка</h3>
                <p>Группа для обсуждения вопросов работы на складской площадке.</p>
            `
        },
        '5.3': {
            title: 'Информация. Читай меня!',
            content: `
                <h3>Информационная группа</h3>
                <p>Группа для размещения важной информации и обновлений.</p>
                
                <div class="image-container">
                    <img src="images/image_23_1.png" alt="Пример информационной группы" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример информационной группы</em></p>
                </div>
            `
        },
        '5.4': {
            title: 'Документы на проверку',
            content: `
                <h3>Группа для проверки документов</h3>
                <p>В эту группу отправляются документы поставщиков из списка на проверку.</p>
                
                <div class="image-container">
                    <img src="images/image_25_1.png" alt="Пример проверки документов" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример проверки документов</em></p>
                </div>
            `
        },
        '5.5': {
            title: 'Акты «М-7»',
            content: `
                <h3>Группа для актов М-7</h3>
                <p>В эту группу отправляется информация о несоответствиях для составления актов М-7.</p>
                
                <div class="image-container">
                    <img src="images/image_26_1.png" alt="Пример акта М-7" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример акта М-7</em></p>
                </div>
            `
        },
        '5.6': {
            title: 'Для ТН для учета',
            content: `
                <h3>Группа для учета ТН</h3>
                <p>В эту группу отправляется информация о выгруженных ТС для учета в 1С.</p>
                
                <div class="image-container">
                    <img src="images/image_28_1.png" alt="Пример учета ТН" style="max-width: 100%; height: auto; border-radius: 10px; margin: 10px 0;">
                    <p><em>Пример учета ТН</em></p>
                </div>
            `
        },
        '5.7': {
            title: 'Проверка заявок перед выдачей',
            content: `
                <h3>Проверка заявок</h3>
                <p>Группа для проверки заявок на выдачу ТМЦ перед их выполнением.</p>
            `
        },
        '5.8': {
            title: 'Выдача ТМЦ по заявкам',
            content: `
                <h3>Выдача по заявкам</h3>
                <p>Группа для координации выдачи ТМЦ по заявкам.</p>
            `
        },
        '5.9': {
            title: 'Потеряшки',
            content: `
                <h3>Группа Потеряшки</h3>
                <p>Группа для поиска и возврата потерянных или забытых предметов.</p>
            `
        },
        // Глава 6: Общие хозяйственные вопросы
        '6.1': {
            title: 'Заявки на канцелярию и расходные материалы',
            content: `
                <h3>Порядок подачи заявок</h3>
                <p>Заявки на канцелярию и расходные материалы подаются в установленном порядке.</p>
            `
        },
        '6.2': {
            title: 'Время труда и отдыха',
            content: `
                <h3>Режим работы</h3>
                <p>Соблюдение установленного режима труда и отдыха.</p>
            `
        },
        '6.3': {
            title: 'Оформление отпуска',
            content: `
                <h3>Порядок оформления отпуска</h3>
                <p>Оформление отпуска и отпуска без сохранения заработной платы.</p>
            `
        },
        '6.4': {
            title: 'Передача документов в учет',
            content: `
                <h3>Документооборот</h3>
                <p>Порядок передачи документов в отдел учета.</p>
            `
        }
    }
};

// Глобальные переменные
let currentView = 'chapters';
let searchResults = [];

// Инициализация приложения
document.addEventListener('DOMContentLoaded', function() {
    renderChapters();
    setupSearch();
    setupPWA();
});

// Отображение глав
function renderChapters() {
    const chaptersView = document.getElementById('chaptersView');
    chaptersView.innerHTML = '';
    
    warehouseData.chapters.forEach(chapter => {
        const chapterCard = document.createElement('div');
        chapterCard.className = 'chapter-card';
        chapterCard.onclick = () => showChapter(chapter.id);
        
        chapterCard.innerHTML = `
            <h2><span class="icon">${chapter.icon}</span> ${chapter.title}</h2>
            <p>${chapter.description}</p>
            <ul class="subchapters">
                ${chapter.subchapters.map(sub => `<li onclick="event.stopPropagation(); showContent('${sub.id}')">${sub.id} ${sub.title}</li>`).join('')}
            </ul>
        `;
        
        chaptersView.appendChild(chapterCard);
    });
}

// Показать содержимое раздела
function showContent(contentId) {
    const content = warehouseData.content[contentId];
    if (!content) {
        alert('Содержимое раздела пока не добавлено');
        return;
    }
    
    document.getElementById('chaptersView').style.display = 'none';
    document.getElementById('contentView').style.display = 'block';
    document.getElementById('contentTitle').textContent = content.title;
    document.getElementById('contentBody').innerHTML = content.content;
    
    currentView = 'content';
}

// Показать главы
function showChapters() {
    document.getElementById('chaptersView').style.display = 'grid';
    document.getElementById('contentView').style.display = 'none';
    currentView = 'chapters';
}

// Настройка поиска
function setupSearch() {
    const searchInput = document.getElementById('searchInput');
    const searchResults = document.getElementById('searchResults');
    
    searchInput.addEventListener('input', function() {
        const query = this.value.toLowerCase().trim();
        
        if (query.length < 2) {
            searchResults.style.display = 'none';
            return;
        }
        
        const results = searchContent(query);
        displaySearchResults(results);
    });
}

// Поиск по содержимому
function searchContent(query) {
    const results = [];
    
    // Поиск по главам и подглавам
    warehouseData.chapters.forEach(chapter => {
        if (chapter.title.toLowerCase().includes(query) || 
            chapter.description.toLowerCase().includes(query)) {
            results.push({
                type: 'chapter',
                title: chapter.title,
                description: chapter.description,
                id: chapter.id
            });
        }
        
        chapter.subchapters.forEach(sub => {
            if (sub.title.toLowerCase().includes(query)) {
                results.push({
                    type: 'subchapter',
                    title: `${sub.id} ${sub.title}`,
                    description: `Раздел: ${chapter.title}`,
                    id: sub.id
                });
            }
        });
    });
    
    // Поиск по содержимому
    Object.keys(warehouseData.content).forEach(key => {
        const content = warehouseData.content[key];
        if (content.title.toLowerCase().includes(query) || 
            content.content.toLowerCase().includes(query)) {
            results.push({
                type: 'content',
                title: content.title,
                description: 'Содержимое раздела',
                id: key
            });
        }
    });
    
    return results.slice(0, 10); // Ограничиваем 10 результатами
}

// Отображение результатов поиска
function displaySearchResults(results) {
    const searchResults = document.getElementById('searchResults');
    
    if (results.length === 0) {
        searchResults.innerHTML = '<div class="search-result"><p>Ничего не найдено</p></div>';
    } else {
        searchResults.innerHTML = results.map(result => `
            <div class="search-result" onclick="handleSearchResult('${result.id}', '${result.type}')">
                <h3>${result.title}</h3>
                <p>${result.description}</p>
            </div>
        `).join('');
    }
    
    searchResults.style.display = 'block';
}

// Обработка результата поиска
function handleSearchResult(id, type) {
    document.getElementById('searchResults').style.display = 'none';
    document.getElementById('searchInput').value = '';
    
    if (type === 'content') {
        showContent(id);
    } else if (type === 'subchapter') {
        showContent(id);
    } else if (type === 'chapter') {
        // Можно добавить логику для показа главы
        showChapters();
    }
}

// Настройка PWA
function setupPWA() {
    let deferredPrompt;
    const installBtn = document.getElementById('installBtn');
    
    window.addEventListener('beforeinstallprompt', (e) => {
        e.preventDefault();
        deferredPrompt = e;
        installBtn.classList.remove('hidden');
    });
    
    installBtn.addEventListener('click', async () => {
        if (deferredPrompt) {
            deferredPrompt.prompt();
            const { outcome } = await deferredPrompt.userChoice;
            console.log(`User response to the install prompt: ${outcome}`);
            deferredPrompt = null;
            installBtn.classList.add('hidden');
        }
    });
    
    window.addEventListener('appinstalled', () => {
        console.log('PWA was installed');
        installBtn.classList.add('hidden');
    });
}

// Регистрация Service Worker
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('sw.js')
            .then((registration) => {
                console.log('SW registered: ', registration);
            })
            .catch((registrationError) => {
                console.log('SW registration failed: ', registrationError);
            });
    });
}
